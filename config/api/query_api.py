...
def searchWords(word, source, language, limits = 10):
    closest_match = Ecdict.objects.filter(word__startswith=word)[:2000]
    matched_word = pd.DataFrame.from_records(closest_match.values())
    
    words = matched_word.word
    collins_score =  matched_word.collins
    oxford = [5*i if i>0 else -1 for i in matched_word.oxford]
    frequency = [i/1000000 if i>0 else -1 for i in matched_word.frequency]
    
    #translation = [i[4] for i in candidates]
    
    score = collins_score+oxford+frequency
...
def getWordAnnotation(user, word, type):
    if existsWordAnnotationDB(user, word, type):
        return getWordAnnotationDB(user, word, type).data
    else:
        return ""
...
def queryNextExerciseWords(user, bookName, language, source, n=2):
    ...
    if len(learningWordsMat)>0:
        ...
        learningWordsDF = learningWordsDF.sort_values(by=['date'])
        learningWordsDF = learningWordsDF.reset_index()
    else:
